...
import { tickStatuses, decayStatuses } from './tickStatuses';
...
describe('status system', () => {
  it('ticks burn damage at turn start', () => {
    ...
    state = applied.state;
    const tick = tickStatuses(state, 'enemy', defaultRegistry);
    expect(tick.events.some((e) => e.type === 'status_tick')).toBe(true);
    expect(tick.events.some((e) => e.type === 'damage_dealt')).toBe(true);
    expect(tick.state.combatants.get('enemy')!.hp).toBeLessThan(100);
  });

  it('decays status at end of turn', () => {
    let state = setup();
    const rng = createSeededRandom(1);
    const applied = applyStatus({
      state,
    ...
    state = applied.state;
    const decay = decayStatuses(state, 'enemy', defaultRegistry);
    expect(decay.state.combatants.get('enemy')!.statuses).toHaveLength(0);
    expect(decay.events.some((e) => e.type === 'status_removed')).toBe(true);
  });
