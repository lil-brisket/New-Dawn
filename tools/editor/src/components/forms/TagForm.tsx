...
import { TagBehaviorReference } from '../BehaviorEditors/TagBehaviorReference';
...
export function TagForm({
  ...
  const behaviors = (draft.behaviors as TagBehavior[] | undefined) ?? [];
  const issues = useDraftValidation('tags', draft);
  ...
  return (
    <div>
      ...
      <section style={sectionCard}>
        <h3 style={sectionTitle}>Values</h3>
        <BehaviorBuilder
          behaviors={behaviors}
          onChange={(b) => set('behaviors', b)}
          lockStructure
        />
      </section>

      <TagBehaviorReference tagId={String(draft.id ?? '')} behaviors={behaviors} />
